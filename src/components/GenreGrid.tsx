...
export const GenreGrid = ({ onGenreSelect, onDirectSearch }: GenreGridProps) => {
    ...
    <div className="p-6 space-y-8">
      <div className="text-center space-y-4">
        ...
        <MovieSearchBar 
          ...
          placeholder="Search any movie to find similar ones... 🔍"
        />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {genres.map((genre, index) => (
          ...
          >
            <Button 
              onClick={() => handleGenreClick(genre.name.split(' ')[0])}
              className={`w-full h-full min-h-[80px] text-lg font-medium transition-all duration-300
                ${selectedGenre === genre.name.split(' ')[0]
                  ? 'bg-[#8B5CF6] text-white shadow-lg'
                  : 'bg-[#1E1E1E] hover:bg-[#2A2A2A] text-[#F5F5F5]'
                }`}
              variant="ghost"
            >
